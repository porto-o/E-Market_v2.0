...
const stripe = Stripe('sk_test_51I76NLIDgbX8kir9GV5rUDxDj0DVlwXg2bb0w20LR2Hs047utSxuomDtSIv8FtitbOUVSYsgk3zkxUsZjcNdw6u500QxT81uMm');
            ...
            if (err) {
                ...
                if (!listData) {
                    ...
                    Restaurant.findOne({codeRes: pin.pin}, (err, restaurantData) => {
                        if (err) {
                            res.status(500).send({message: "Error al buscar el restaurante "});
                        } else {
                            if (!restaurantData) {
                    ...
                    MiLista.findOne({Comensal: id}, (err, existingListData) => {
                        if (err) {
                            ...
                            Restaurant.findOne({codeRes: pin.pin}, (err, restaurantData) => {
                                if (err) {
                                    res.status(500).send({message: "Error al buscar el restaurante, intente más tarde."});
                                } else {
                                    if (!restaurantData) {
                                        console.log(("No existe el restaurante"));
                                        res.status(404).send({message: "El cógido ingresado no corresponde a ningún restaurante."})
                                        ...
                                        if (indexes.length > 0) {
                                            ...
                                            res.status(500).send({message: "Ya tienes ese restaurante agregado a tu lista."})
                                        } else {
                                            array.push(element)
                                            console.log(array)
                                            MiLista.findOneAndUpdate({Comensal: id}, {Restaurantes: array}, (Err, resUpdate) => {
                                                if(Err){
                                                    console.log("error al actualizar el array de objetos ", Err);
                                                }else{
        ...
        if(err){
            ...
            Menu.findOne({"Restaurante": id}, (err, resMenu) => {
                if(err){
                    ...
                    if(!resMenu){
                        res.status(500).send({message: "El restaurante aún no tiene un menú. Comunícate con el gerente."})
                    }else{
        ...
        if (newPassword == null || newPassword == "") {
            ...
            bcrypt.hash(newPassword, null, null, function (err, hash) {
                if (err) {
                    ...
                    Comensal.findByIdAndUpdate({_id: idComensal}, {password: hash}, (err, resUpdate) => {
                        if (err) {
...
const ChangeName = (req, res) => {
    ...
    if (idComensal == null || idComensal == "") {
        ...
        //Enviar mensaje l cliente
            ...
            Comensal.findByIdAndUpdate({_id: idComensal},{userName: newName}, (err, resUpdate) => {
                if (err) {
                    ...
                    if(err){
                        ...
                        if(!deleteData){
                            ...
                            for(let i = 0; i < miLista.length ; i++){
                                if(miLista[i].restaurantName.valueOf() == resData.userName.valueOf()){
                                    console.log(miLista[i].restaurantName)
                                    n = i
                                    var restaurantes = 'Restaurante['+n.valueOf()+']'
                                    MiLista.findOneAndDelete({restaurantes: miLista[i].restaurantName.valueOf()})
                                }else{
                ...
                if (errorRFO) {
                    ...
                    if (!resRFO) {
                        ...
                        Menu.findOne({Restaurante: idRestaurant}, (errorMFO, resMFO) => {
                            if (errorRFO) {
                                console.log("Error en el servidor al buscar el menú del restaurante restaurante")
                                //res.status(500).send({message: "Error en el servidor"})
                            } else {
                                if (!resMFO) {
                                    ...
                                    for (let j = 0; j < dishes.length; j++) {
                                        for (let i = 0; i < restaurantMenuArray.length; i++) {
                                            if (restaurantMenuArray[i].nombre == dishes[j]) {
                                                ...
                                                obj = {nombre: dishes[j], precio: restaurantMenuArray[i].precio}
                                                dishesObject.push(obj);
                                            }
                                        }
                                    }
                                    OrdersModel.findOne({Comensal: comensal}, (err, resSearch) => {
                                        if (err) {
                                            ...
                                            if (!resSearch) {
                                                ...
                                                order.save((err, resSave) => {
                                                    if (err) {
                                                        ...
                                                        res.status(500).send({message: "No se logro generar la orden"});
                                                    } else {
                                                        console.log("Orden generada exitosamente");
                                                        res.status(200).send({message: "Orden enviada"});
                                                    }
                                                ...
                                                OrdersModel.findOneAndUpdate({Comensal: comensal}, {Platillos: arrayUpdateDishes, Total: newTotal}, (error, resUpdate) => {
                                                    if (error) {
                                                        console.log("Error al actualizar la orden", error);
                                                        res.status(500).send({message: "Error al actualizar la orden"});
                                                    } else {
                                                        console.log("Orden actualizada");
                                                        res.status(200).send({message: "Orden actualizada"});
                                                    }
                                                });
                                            }
                                        }
    ...
    TicketsModel.find({Comensal: nombre}, (err,resOrders) => {
        if(err){
            ...
            if(!resOrders || resOrders == ""){
                res.status(500).send({message: "Aún no haz realizado ninguna compra."});
            }else{
                ...
                OrdersModel.findOne({Comensal: idComensal}, (error, resOMFO) => {
                    if (error) {
                        ...
                        if (!resOMFO) {
                            ...
                            Restaurant.findOne({userName: restaurantName}, (error1, resRFO) => {
                                if (error) {
                                    ...
                                    if (!resOMFO) {
                                        ...
                                        const idRestaurant = resRFO._id;
                                        Menu.findOne({Restaurante: idRestaurant}, (error2, resMFO) => {
                                            if (error2) {
                                                ...
                                                if (!resMFO) {
                                                    ...
                                                    for (let i = 0; i < dishesObject.length; i++) {
                                                        for (let j = 0; j < restaurantMenuArray.length; j++) {
                                                            console.log(dishesObject[i].nombre == restaurantMenuArray[j].nombre);
                                                            if (dishesObject[i].nombre == restaurantMenuArray[j].nombre) {
                                                                var sumaVentas = parseFloat(restaurantMenuArray[j].ventas) + 1;
                                                                Menu.updateOne({"Menu.nombre": dishesObject[i].nombre}, {$set: {"Menu.$.ventas": sumaVentas.valueOf()}}, (erro,resup) => {
                                                                    if (erro) {
                                                                        console.log("No se actualizo la venta unu", erro)
                                                                    } else {
                                                                        console.log("Actualización de ventas exitosa")
                                                                    }
                                                                });
                                                            }
                                                        }
                                                    ...
                                                    */
                                                    ...
                                                    OrdersModel.findOneAndDelete({Comensal: idComensal}, (error5, resOMFOAD) => {
                                                        if (error5) {
                                                            console.log("Error al eliminar la orden del comensal");
                                                        } else {
...
const deleteDishFromOrder = (req, res)  => {}
...
const getStatus = (req,res) => {
    ...
    OrdersModel.findOne({"Comensal": id}, (err,statusData) => {
        if(err){
            ...
            if(!statusData){
                res.status(500).send({message: "No tienes ninguna orden en progreso"})
            }else{
        ...
        if(err){
            ...
            if(!menuData){
                ...
                const content = '<div>##########</div>    \n' +
                    ...
                    '<div>\n' +
                    '    <ul>\n' +
                    '        <li>Artículos: '+ platillos.map((item) => (
                                        item.nombre
                                    ))+'</li>\n' +
                    '        <li>Total: '+total+'</li>\n' +
                    '    </ul>\n' +
                ...
                PDF.create(content).toFile('./ticket.pdf', (err,resTicket) => {
                    if (err){
                        console.log(err);
                    } else {
                        const Firma = require('./sign')
                        ...
                        for(let i = 0; i <= firma.length; i++){
                                if(i % 90 == 0){
                                    firmaHTML += firma.charAt(i)+'<br></br>'
                                }else{
                                    firmaHTML += firma.charAt(i)
                                }
                        }
                        ...
                        const content2 = '<div>##########</div>    \n' +
                            ...
                            '    </ul>\n' +
                            '</div>\n' +
                            '<p>Firma Digital: '+firmaHTML+'</p>' +
                            '<div><img src='+qr+'></div>'

                        PDF.create(content2).toFile('./ticketFirmado.pdf', (err, resFirmado) => {
                            if(err){
                                console.log(err)
                            }else{
                                console.log("Firmado generado en: "+resFirmado)
                            }
                        })
                                ...
                                if (error4) {
                                    ...
                                    ticket.save((error5, resTS) => {
                                        if (error5) {
                                            ...
                                            console.log("Orden guardad exitosamente en el restaurante");
                                            OrdersModel.findOneAndDelete({Comensal: idUser}, (error5, resOMFOAD) => {
                                                if (error5) {
                                                    console.log("Error al eliminar la orden del comensal");
                                                } else {
                                                    console.log("Orden eliminada despues del pago");
...
const verificarFirma = (req,res) => {
    ...
    if(result){
        ...
        console.log(result)
        res.status(500).send({message: "Error 406, la firma no es válida para el ticket generado."})
    }
